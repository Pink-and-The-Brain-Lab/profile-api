...
export class RabbitMqMessagesProducerService {
    async sendDataToAPI<T>(data: T, queue: RabbitMqQueues) {
        ...
        const channel = await rabbitmq.createChannel(RabbitMqQueues.PROFILE_RESPONSE_QUEUE);
        ...
        channel.sendToQueue(
            ...
            {
                correlationId,
                replyTo: RabbitMqQueues.PROFILE_RESPONSE_QUEUE
            }
        );
    ...
    private awaitApiResponse(
        ...
        return new Promise(resolve => {
            channel.consume(RabbitMqQueues.PROFILE_RESPONSE_QUEUE, (message: ConsumeMessage | null) => {
